...
/** Behaviour class of the combination the tree and console view. */
final class TasksTreeConsoleBehaviour implements Behavior<TasksTreeConsoleModel> {
  ...
  private void ensureSpaceForOneFinishedTask() {
    while (model.getTopLevelFinishedTasks().size() >= TasksTreeConsoleModel.MAX_FINISHED_TASKS) {
      ...
      if (consoleView == null) {
        throw new IllegalStateException(
            "Finished task `" + task.getName() + "` doesn't have " + "corresponding console view");
      }
